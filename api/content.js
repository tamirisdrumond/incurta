// api/content.js
// Serves the protected questionnaire shell HTML after password verification.
// The app HTML is never in the initial page source.
//
// WHY LANG IS PASSED HERE (not injected client-side):
//   The HTML is a raw string injected into the DOM via innerHTML. By the time
//   the browser renders it, the DOMContentLoaded event has already fired and
//   applyTranslations() will not re-run. The server must pre-translate.
//
// CONTRACT:
//   POST /api/content
...
function buildShellHtml(s) {
  // `s` is the full string map for the requested language.
  // All user-visible text comes from s.* — no hardcoded strings here.
  return `
